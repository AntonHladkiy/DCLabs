    ...
    public static void main(String[] args) {
        ...
        for (int i = 10; i < 15; i++) {
            threads[i] = new Thread(
                    new CheckForTripRunnable(tripGraph, random.nextInt(150), random.nextInt(150)));
        }
        for (int i = 15; i < 20; i++) {
            threads[i] = new Thread(new RemoveCityRunnable(tripGraph, random.nextInt(200)));
        }
        for (int i = 20; i < 25; i++) {
            threads[i] = new Thread(new RemoveTripRunnable(tripGraph, random.nextInt(200), random.nextInt(200)));
        }
        for (int i = 25; i < 30; i++) {
            threads[i] = new Thread(
                    new CheckForTripRunnable(tripGraph, random.nextInt(200), random.nextInt(200)));
        }
