...
public class CheckerRunnable implements Runnable {
    ...
    private final int[] aCount = new int[Main.getStringBuilders().length];
    private final int[] bCount = new int[Main.getStringBuilders().length];

    @Override
    public void run() {
        Arrays.fill(aCount, 0);
        Arrays.fill(bCount, 0);
        for (int i = 0; i < Main.getStringBuilders().length; i++) {
            countAB(Main.getStringBuilders()[i], i);
        }

        if (checkIfDone()) {
            for (int i = 0; i < Main.getThreads().length; i++) {
                Main.getThreads()[i].interrupt();
                System.out.println(Main.getStringBuilders()[i].toString());
            }
        }
    }
